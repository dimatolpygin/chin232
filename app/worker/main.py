...
class WorkerSettings:
    ...
    max_tries = 1
    # Выкат не должен обрывать круг на полуслове. По умолчанию arq на SIGTERM
    # рубит работающие задачи мгновенно: юзер отправил голосовое, слот с него
    # списан, а ответа нет и не будет — повтор запрещён строкой выше. С этим
    # числом воркер перестаёт брать новые задачи и даёт текущим договорить.
    # В паре с ним stop_grace_period в compose: docker иначе прибьёт контейнер
    # через свои десять секунд, не дожидаясь.
