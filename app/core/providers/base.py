...
@dataclass(slots=True)
class Voice:
    """Голос из списка настроек.

    Идентификатор принадлежит конкретному сервису озвучки, поэтому список
    голосов живёт у провайдера, а не в общем конфиге: при смене сервиса
    старые идентификаторы становятся мусором, и подставлять их нельзя.
    """

    id: str
    title: str
    # Короткое описание для строки под названием: женский, мужской, темп.
    note: str
...
class TTSProvider(ABC):
    ...
    name: str
    # Голоса, которые видит юзер в настройках. Пустой список — выбора нет,
    # раздел покажет только то, что умеет провайдер.
    VOICES: tuple[Voice, ...] = ()
